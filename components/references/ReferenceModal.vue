<template>
  <div
    ...
    tabindex="-1"
    role="dialog"
    aria-labelledby="referenceModal"
    :reference="selectedRef"
  >
    <div class="modal-dialog modal-lg" role="document">
      <div v-if="selectedRef.paper" class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">
            <vote :voteable="selectedRef" :signed-in="signedIn" />
            {{ selectedRef.paper.title }}
            <a class="modal-external-link" :href="selectedRef.paper.direct_link" target="_blank" />
          </h4>
          <mini-bib :r="selectedRef" />
          <div>
            <span v-if="selectedRef.paper.tag_list == 0 && editsAllowed">Add tags</span>
            <a v-for="(tag,index) in selectedRef.paper.tag_list" class="tag">
              {{ tag }}
              <button
                v-show="showTagForm"
                class="tag-remove"
            ...
            <a v-if="editsAllowed" class="edit-btn" @click="showTagForm = !showTagForm" />
        ...
        <div class="modal-body">
          <div class="modal-section">
            ...
            <div v-if="selectedRef.notes.length == 0" class="nothing-yet">
              No notes yet...
            </div>
            <div v-for="(note,index) in selectedRef.notes.slice(0,showAllNotes ? selectedRef.notes.length + 1 : 3)" class="note">
              ...
              <div v-else>
                <div class="form-group">
                  <textarea v-model="note.edit_form" class="form-control" />
                </div>
                <div class="form-btns">
                  <button class="submit-btn" @click="updateNote(note)">
                    Submit
                  </button>
          ...
          <div class="modal-section">
            ...
            <div v-if="!hasAbstract" v-show="!editAbstract" class="nothing-yet">
            ...
            <div v-if="editsAllowed" v-show="editAbstract">
              ...
              <div class="form-btns">
                <button class="submit-btn" @click="submitAbstract(selectedRef)">
                  Submit
                </button>
                <a class="cancel-link" @click="cancelAbstractForm">Cancel</a>
              </div>
            </div>
            <abstract v-if="hasAbstract" v-show="!editAbstract" :abstract="selectedRef.paper.abstract" />
          </div>
          <div class="modal-section">
            <div class="modal-section-header">
              Details
            ...
            <ul class="list-unstyled">
              ...
              <li v-if="selectedRef.paper.pubmed_id != null">
                Pubmed:
                <a
                  :href="'https://www.ncbi.nlm.nih.gov/pubmed/'+selectedRef.paper.pubmed_id"
                  target="_blank"
                >{{ selectedRef.paper.pubmed_id }}</a>
              </li>
              <div v-if="selectedRef.paper.links.length > 0">
                Direct Links:
              </div>
...
export default {
  ...
  methods: {
    ...
    submitTags () {
      let tag_list = this.selectedRef.paper.tag_list
